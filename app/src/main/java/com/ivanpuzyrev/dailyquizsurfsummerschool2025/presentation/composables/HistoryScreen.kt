                    ...
                    LazyColumn(modifier = Modifier.weight(1f)) {
                        items(items = gameResults, key = { it.id }) { gameResult ->
                            GameResultCard(
                                ...
                                onDeleteHistoryRecordClicked = {
                                    isAlertDialog = true
                                    viewModel.deleteGameResult(it)
                                },
...
@Composable
fun DeletedRecordAlertDialog(onDismiss: (Boolean) -> Unit) {
    AlertDialog(
        ...
        confirmButton = {
            TextButton(onClick = { onDismiss(false) }) {
                Text(text = "ЗАКРЫТЬ", color = Indigo, fontSize = 16.sp)
            }
        }
    )
}
